...
function unload(featureMetadataLoader) {
  featureMetadataLoader._bufferViewLoaders.forEach(function (bufferViewLoader) {
    ResourceCache.unload(bufferViewLoader);
  });
  featureMetadataLoader._bufferViewLoaders = [];

  featureMetadataLoader._textureLoaders.forEach(function (textureLoader) {
    ResourceCache.unload(textureLoader);
  });
  featureMetadataLoader._textureLoaders = [];

  if (defined(featureMetadataLoader._schemaLoader)) {
    ResourceCache.unload(featureMetadataLoader._schemaLoader);
  }
  featureMetadataLoader._schemaLoader = undefined;

  featureMetadataLoader._featureMetadata = undefined;
}
 ...
 *
 * @see GltfFeatureMetadataLoader#destroy
 */
GltfFeatureMetadataLoader.prototype.isDestroyed = function () {
  return false;
};
 ...
 *
 * @example
 * featureMetadataLoader = featureMetadataLoader && featureMetadataLoader.destroy();
 *
 * @see GltfFeatureMetadataLoader#isDestroyed
 */
GltfFeatureMetadataLoader.prototype.destroy = function () {
  unload(this);
