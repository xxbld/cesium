...
function loadFromDraco(indexBufferLoader) {
  ...
  dracoLoader.promise
    .then(function () {
      if (indexBufferLoader._state === ResourceLoaderState.DESTROYED) {
        unload(indexBufferLoader);
        return;
      }
...
function unload(indexBufferLoader) {
  if (defined(indexBufferLoader._indexBuffer)) {
    indexBufferLoader._indexBuffer.destroy();
  }

  var resourceCache = indexBufferLoader._resourceCache;

  if (defined(indexBufferLoader._bufferViewLoader)) {
    resourceCache.unload(indexBufferLoader._bufferViewLoader);
  }

  if (defined(indexBufferLoader._dracoLoader)) {
    resourceCache.unload(indexBufferLoader._dracoLoader);
  }

  indexBufferLoader._bufferViewLoader = undefined;
  indexBufferLoader._dracoLoader = undefined;
  indexBufferLoader._typedArray = undefined;
  indexBufferLoader._indexBuffer = undefined;
  indexBufferLoader._gltf = undefined;
}
 ...
 *
 * @see GltfIndexBufferLoader#destroy
 */
GltfIndexBufferLoader.prototype.isDestroyed = function () {
  return false;
};
 ...
 *
 * @example
 * indexBufferLoader = indexBufferLoader && indexBufferLoader.destroy();
 *
 * @see GltfIndexBufferLoader#isDestroyed
 */
GltfIndexBufferLoader.prototype.destroy = function () {
  unload(this);
